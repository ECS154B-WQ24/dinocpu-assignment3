...
class PipelinedCPU(implicit val conf: CPUConfig) extends BaseCPU {
  ...
  val jump_pc = Wire(UInt(64.W))
  jump_pc := DontCare // Remove when connected
  ...
  // Update the PC:
  // (Part I) Choose between PC+4 and jumppc from the JumpPcGenerator to update PC
  ...
  // Connect the JumpDetectionUnit control wire
  ...
  // Send jump_pc back to the Fetch stage
